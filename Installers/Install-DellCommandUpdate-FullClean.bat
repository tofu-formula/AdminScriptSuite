...
ECHO Install App: Dell Command Update
ECHO Install Method: Multiple steps
ECHO 1. Attempt clean uninstall of pre-existing installations of DCU
...
ECHO 1. Attempt clean uninstall of pre-existing installations of DCU
Powershell.exe -executionpolicy remotesigned -File "%REPOROOT%/Uninstallers/General_Uninstaller.ps1" -AppName "Dell.CommandUpdate" -UninstallType "All" -WorkingDirectory "%WORKINGDIR%"
Powershell.exe -executionpolicy remotesigned -File "%REPOROOT%/Uninstallers/General_Uninstaller.ps1" -AppName "Dell.CommandUpdate.Universal" -UninstallType "All" -WorkingDirectory "%WORKINGDIR%"
REM Add more AppNames if necessary
ECHO.
...
ECHO 3. Install DCU using WinGet
Powershell.exe -executionpolicy remotesigned -File "%~dp0General_WinGet_Installer.ps1" -AppName "Dell_Command_Update" -AppID "Dell.CommandUpdate" -WorkingDirectory "%WORKINGDIR%"